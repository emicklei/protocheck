// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.2
// 	protoc        v5.29.3
// source: person.proto

package golang

import (
	_ "github.com/emicklei/protocheck"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type Person_Gender int32

const (
	Person_UNKNOWN Person_Gender = 0
	Person_MALE    Person_Gender = 1
	Person_FEMALE  Person_Gender = 2
)

// Enum value maps for Person_Gender.
var (
	Person_Gender_name = map[int32]string{
		0: "UNKNOWN",
		1: "MALE",
		2: "FEMALE",
	}
	Person_Gender_value = map[string]int32{
		"UNKNOWN": 0,
		"MALE":    1,
		"FEMALE":  2,
	}
)

func (x Person_Gender) Enum() *Person_Gender {
	p := new(Person_Gender)
	*p = x
	return p
}

func (x Person_Gender) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Person_Gender) Descriptor() protoreflect.EnumDescriptor {
	return file_person_proto_enumTypes[0].Descriptor()
}

func (Person_Gender) Type() protoreflect.EnumType {
	return &file_person_proto_enumTypes[0]
}

func (x Person_Gender) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

type Person struct {
	state                     protoimpl.MessageState  `protogen:"opaque.v1"`
	xxx_hidden_Name           string                  `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	xxx_hidden_MiddleName     *string                 `protobuf:"bytes,2,opt,name=middle_name,json=middleName,proto3,oneof" json:"middle_name,omitempty"`
	xxx_hidden_Surname        string                  `protobuf:"bytes,3,opt,name=surname,proto3" json:"surname,omitempty"`
	xxx_hidden_BirthDate      *timestamppb.Timestamp  `protobuf:"bytes,4,opt,name=birth_date,json=birthDate,proto3" json:"birth_date,omitempty"`
	xxx_hidden_Health         *Person_Health          `protobuf:"bytes,5,opt,name=health,proto3" json:"health,omitempty"`
	xxx_hidden_Identification isPerson_Identification `protobuf_oneof:"identification"`
	xxx_hidden_Nicknames      []string                `protobuf:"bytes,8,rep,name=nicknames,proto3" json:"nicknames,omitempty"`
	xxx_hidden_Pets           *[]*Pet                 `protobuf:"bytes,9,rep,name=pets,proto3" json:"pets,omitempty"`
	xxx_hidden_Attributes     map[string]string       `protobuf:"bytes,10,rep,name=attributes,proto3" json:"attributes,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	xxx_hidden_Favorites      map[string]*Pet         `protobuf:"bytes,11,rep,name=favorites,proto3" json:"favorites,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	xxx_hidden_NoCheckDate    *timestamppb.Timestamp  `protobuf:"bytes,12,opt,name=no_check_date,json=noCheckDate,proto3" json:"no_check_date,omitempty"`
	xxx_hidden_Gender         Person_Gender           `protobuf:"varint,13,opt,name=gender,proto3,enum=golang.Person_Gender" json:"gender,omitempty"`
	XXX_raceDetectHookData    protoimpl.RaceDetectHookData
	XXX_presence              [1]uint32
	unknownFields             protoimpl.UnknownFields
	sizeCache                 protoimpl.SizeCache
}

func (x *Person) Reset() {
	*x = Person{}
	mi := &file_person_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Person) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Person) ProtoMessage() {}

func (x *Person) ProtoReflect() protoreflect.Message {
	mi := &file_person_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Person) GetName() string {
	if x != nil {
		return x.xxx_hidden_Name
	}
	return ""
}

func (x *Person) GetMiddleName() string {
	if x != nil {
		if x.xxx_hidden_MiddleName != nil {
			return *x.xxx_hidden_MiddleName
		}
		return ""
	}
	return ""
}

func (x *Person) GetSurname() string {
	if x != nil {
		return x.xxx_hidden_Surname
	}
	return ""
}

func (x *Person) GetBirthDate() *timestamppb.Timestamp {
	if x != nil {
		return x.xxx_hidden_BirthDate
	}
	return nil
}

func (x *Person) GetHealth() *Person_Health {
	if x != nil {
		return x.xxx_hidden_Health
	}
	return nil
}

func (x *Person) GetEmail() string {
	if x != nil {
		if x, ok := x.xxx_hidden_Identification.(*person_Email); ok {
			return x.Email
		}
	}
	return ""
}

func (x *Person) GetPhone() string {
	if x != nil {
		if x, ok := x.xxx_hidden_Identification.(*person_Phone); ok {
			return x.Phone
		}
	}
	return ""
}

func (x *Person) GetNicknames() []string {
	if x != nil {
		return x.xxx_hidden_Nicknames
	}
	return nil
}

func (x *Person) GetPets() []*Pet {
	if x != nil {
		if x.xxx_hidden_Pets != nil {
			return *x.xxx_hidden_Pets
		}
	}
	return nil
}

func (x *Person) GetAttributes() map[string]string {
	if x != nil {
		return x.xxx_hidden_Attributes
	}
	return nil
}

func (x *Person) GetFavorites() map[string]*Pet {
	if x != nil {
		return x.xxx_hidden_Favorites
	}
	return nil
}

func (x *Person) GetNoCheckDate() *timestamppb.Timestamp {
	if x != nil {
		return x.xxx_hidden_NoCheckDate
	}
	return nil
}

func (x *Person) GetGender() Person_Gender {
	if x != nil {
		return x.xxx_hidden_Gender
	}
	return Person_UNKNOWN
}

func (x *Person) SetName(v string) {
	x.xxx_hidden_Name = v
}

func (x *Person) SetMiddleName(v string) {
	x.xxx_hidden_MiddleName = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 12)
}

func (x *Person) SetSurname(v string) {
	x.xxx_hidden_Surname = v
}

func (x *Person) SetBirthDate(v *timestamppb.Timestamp) {
	x.xxx_hidden_BirthDate = v
}

func (x *Person) SetHealth(v *Person_Health) {
	x.xxx_hidden_Health = v
}

func (x *Person) SetEmail(v string) {
	x.xxx_hidden_Identification = &person_Email{v}
}

func (x *Person) SetPhone(v string) {
	x.xxx_hidden_Identification = &person_Phone{v}
}

func (x *Person) SetNicknames(v []string) {
	x.xxx_hidden_Nicknames = v
}

func (x *Person) SetPets(v []*Pet) {
	x.xxx_hidden_Pets = &v
}

func (x *Person) SetAttributes(v map[string]string) {
	x.xxx_hidden_Attributes = v
}

func (x *Person) SetFavorites(v map[string]*Pet) {
	x.xxx_hidden_Favorites = v
}

func (x *Person) SetNoCheckDate(v *timestamppb.Timestamp) {
	x.xxx_hidden_NoCheckDate = v
}

func (x *Person) SetGender(v Person_Gender) {
	x.xxx_hidden_Gender = v
}

func (x *Person) HasMiddleName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *Person) HasBirthDate() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_BirthDate != nil
}

func (x *Person) HasHealth() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Health != nil
}

func (x *Person) HasIdentification() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Identification != nil
}

func (x *Person) HasEmail() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_Identification.(*person_Email)
	return ok
}

func (x *Person) HasPhone() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_Identification.(*person_Phone)
	return ok
}

func (x *Person) HasNoCheckDate() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_NoCheckDate != nil
}

func (x *Person) ClearMiddleName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_MiddleName = nil
}

func (x *Person) ClearBirthDate() {
	x.xxx_hidden_BirthDate = nil
}

func (x *Person) ClearHealth() {
	x.xxx_hidden_Health = nil
}

func (x *Person) ClearIdentification() {
	x.xxx_hidden_Identification = nil
}

func (x *Person) ClearEmail() {
	if _, ok := x.xxx_hidden_Identification.(*person_Email); ok {
		x.xxx_hidden_Identification = nil
	}
}

func (x *Person) ClearPhone() {
	if _, ok := x.xxx_hidden_Identification.(*person_Phone); ok {
		x.xxx_hidden_Identification = nil
	}
}

func (x *Person) ClearNoCheckDate() {
	x.xxx_hidden_NoCheckDate = nil
}

const Person_Identification_not_set_case case_Person_Identification = 0
const Person_Email_case case_Person_Identification = 6
const Person_Phone_case case_Person_Identification = 7

func (x *Person) WhichIdentification() case_Person_Identification {
	if x == nil {
		return Person_Identification_not_set_case
	}
	switch x.xxx_hidden_Identification.(type) {
	case *person_Email:
		return Person_Email_case
	case *person_Phone:
		return Person_Phone_case
	default:
		return Person_Identification_not_set_case
	}
}

type Person_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// with per field state checks
	Name       string
	MiddleName *string
	Surname    string
	BirthDate  *timestamppb.Timestamp
	Health     *Person_Health
	// oneof with field checks
	// test with https://playcel.undistro.io/

	// Fields of oneof xxx_hidden_Identification:
	Email *string
	Phone *string
	// -- end of xxx_hidden_Identification
	// repeated with field check
	Nicknames []string
	// repeated proto message with  check
	Pets []*Pet
	// map with field check
	Attributes map[string]string
	// map with proto message value with check
	Favorites   map[string]*Pet
	NoCheckDate *timestamppb.Timestamp
	Gender      Person_Gender
}

func (b0 Person_builder) Build() *Person {
	m0 := &Person{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Name = b.Name
	if b.MiddleName != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 12)
		x.xxx_hidden_MiddleName = b.MiddleName
	}
	x.xxx_hidden_Surname = b.Surname
	x.xxx_hidden_BirthDate = b.BirthDate
	x.xxx_hidden_Health = b.Health
	if b.Email != nil {
		x.xxx_hidden_Identification = &person_Email{*b.Email}
	}
	if b.Phone != nil {
		x.xxx_hidden_Identification = &person_Phone{*b.Phone}
	}
	x.xxx_hidden_Nicknames = b.Nicknames
	x.xxx_hidden_Pets = &b.Pets
	x.xxx_hidden_Attributes = b.Attributes
	x.xxx_hidden_Favorites = b.Favorites
	x.xxx_hidden_NoCheckDate = b.NoCheckDate
	x.xxx_hidden_Gender = b.Gender
	return m0
}

type case_Person_Identification protoreflect.FieldNumber

func (x case_Person_Identification) String() string {
	md := file_person_proto_msgTypes[0].Descriptor()
	if x == 0 {
		return "not set"
	}
	return protoimpl.X.MessageFieldStringOf(md, protoreflect.FieldNumber(x))
}

type isPerson_Identification interface {
	isPerson_Identification()
}

type person_Email struct {
	Email string `protobuf:"bytes,6,opt,name=email,proto3,oneof"`
}

type person_Phone struct {
	Phone string `protobuf:"bytes,7,opt,name=phone,proto3,oneof"`
}

func (*person_Email) isPerson_Identification() {}

func (*person_Phone) isPerson_Identification() {}

// second message in same file
type Pet struct {
	state           protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Kind string                 `protobuf:"bytes,1,opt,name=kind,proto3" json:"kind,omitempty"`
	xxx_hidden_Name string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *Pet) Reset() {
	*x = Pet{}
	mi := &file_person_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Pet) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Pet) ProtoMessage() {}

func (x *Pet) ProtoReflect() protoreflect.Message {
	mi := &file_person_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Pet) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *Pet) GetName() string {
	if x != nil {
		return x.xxx_hidden_Name
	}
	return ""
}

func (x *Pet) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *Pet) SetName(v string) {
	x.xxx_hidden_Name = v
}

type Pet_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Kind string
	Name string
}

func (b0 Pet_builder) Build() *Pet {
	m0 := &Pet{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_Name = b.Name
	return m0
}

// message without checks
type Group struct {
	state           protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Name string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *Group) Reset() {
	*x = Group{}
	mi := &file_person_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Group) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Group) ProtoMessage() {}

func (x *Group) ProtoReflect() protoreflect.Message {
	mi := &file_person_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Group) GetName() string {
	if x != nil {
		return x.xxx_hidden_Name
	}
	return ""
}

func (x *Group) SetName(v string) {
	x.xxx_hidden_Name = v
}

type Group_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Name string
}

func (b0 Group_builder) Build() *Group {
	m0 := &Group{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Name = b.Name
	return m0
}

// nested message
type Person_Health struct {
	state                  protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Weight      int32                  `protobuf:"varint,1,opt,name=weight,proto3" json:"weight,omitempty"`
	xxx_hidden_AvgHartRate float64                `protobuf:"fixed64,2,opt,name=avg_hart_rate,json=avgHartRate,proto3" json:"avg_hart_rate,omitempty"`
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *Person_Health) Reset() {
	*x = Person_Health{}
	mi := &file_person_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Person_Health) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Person_Health) ProtoMessage() {}

func (x *Person_Health) ProtoReflect() protoreflect.Message {
	mi := &file_person_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Person_Health) GetWeight() int32 {
	if x != nil {
		return x.xxx_hidden_Weight
	}
	return 0
}

func (x *Person_Health) GetAvgHartRate() float64 {
	if x != nil {
		return x.xxx_hidden_AvgHartRate
	}
	return 0
}

func (x *Person_Health) SetWeight(v int32) {
	x.xxx_hidden_Weight = v
}

func (x *Person_Health) SetAvgHartRate(v float64) {
	x.xxx_hidden_AvgHartRate = v
}

type Person_Health_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Weight      int32
	AvgHartRate float64
}

func (b0 Person_Health_builder) Build() *Person_Health {
	m0 := &Person_Health{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Weight = b.Weight
	x.xxx_hidden_AvgHartRate = b.AvgHartRate
	return m0
}

var File_person_proto protoreflect.FileDescriptor

var file_person_proto_rawDesc = []byte{
	,
}

var file_person_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_person_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_person_proto_goTypes = []any{
	(Person_Gender)(0),            // 0: golang.Person.Gender
	(*Person)(nil),                // 1: golang.Person
	(*Pet)(nil),                   // 2: golang.Pet
	(*Group)(nil),                 // 3: golang.Group
	(*Person_Health)(nil),         // 4: golang.Person.Health
	nil,                           // 5: golang.Person.AttributesEntry
	nil,                           // 6: golang.Person.FavoritesEntry
	(*timestamppb.Timestamp)(nil), // 7: google.protobuf.Timestamp
}
var file_person_proto_depIdxs = []int32{
	7, // 0: golang.Person.birth_date:type_name -> google.protobuf.Timestamp
	4, // 1: golang.Person.health:type_name -> golang.Person.Health
	2, // 2: golang.Person.pets:type_name -> golang.Pet
	5, // 3: golang.Person.attributes:type_name -> golang.Person.AttributesEntry
	6, // 4: golang.Person.favorites:type_name -> golang.Person.FavoritesEntry
	7, // 5: golang.Person.no_check_date:type_name -> google.protobuf.Timestamp
	0, // 6: golang.Person.gender:type_name -> golang.Person.Gender
	2, // 7: golang.Person.FavoritesEntry.value:type_name -> golang.Pet
	8, // [8:8] is the sub-list for method output_type
	8, // [8:8] is the sub-list for method input_type
	8, // [8:8] is the sub-list for extension type_name
	8, // [8:8] is the sub-list for extension extendee
	0, // [0:8] is the sub-list for field type_name
}

func init() { file_person_proto_init() }
func file_person_proto_init() {
	if File_person_proto != nil {
		return
	}
	file_person_proto_msgTypes[0].OneofWrappers = []any{
		(*person_Email)(nil),
		(*person_Phone)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_person_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_person_proto_goTypes,
		DependencyIndexes: file_person_proto_depIdxs,
		EnumInfos:         file_person_proto_enumTypes,
		MessageInfos:      file_person_proto_msgTypes,
	}.Build()
	File_person_proto = out.File
	file_person_proto_rawDesc = nil
	file_person_proto_goTypes = nil
	file_person_proto_depIdxs = nil
}
